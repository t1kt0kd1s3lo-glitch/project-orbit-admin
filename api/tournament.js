...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      const r = await httpsRequest(`${ORBIT}/api/v1/events/Fortnite/download/World`);
  ...
  if (req.method === "POST") {
    ...
    try {
      const r = await httpsRequest(`${ORBIT}/dashboard/tournament/start`, {
        method: "POST",
  ...
  if (req.method === "DELETE") {
    try {
      const r = await httpsRequest(`${ORBIT}/dashboard/tournament/end`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Orbit-Admin": TOKEN },
      });
      if (!r.ok) throw new Error("Orbit " + r.status);
